...
df_pca <- fread("Supp_Fig5a.txt",header=TRUE,check.names=FALSE)
title <- colnames(df_pca)
df_pca$pop <- ifelse(df_pca$superpop=="GTEX","GTEX",df_pca$pop)
df_pca <- rbind(df_pca[df_pca$pop %in% c("CDX","CHB","CHS","JPT","KHV","GTEX"),],
                df_pca[df_pca$pop=="GTOP",])

df_pca$pop <- ifelse(df_pca$pop %in% c("CDX","CHB","CHS","JPT","KHV","GTEX"),df_pca$pop,"GTOP")

colors_0 <- c("CDX"="#f39b7f","GTOP"="#873325","CHB"="#4dbbd5","GTEX"="#236f73",
