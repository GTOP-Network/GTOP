...
# supp.Figure.36b join finemapping res of tuQTL ---------------------------


df_plot <- df_plotall %>% dplyr::filter(qtltype=="tuQTL")
order <- c(
  "Adrenal_Gland","Adipose","Liver","Gallbladder","Pancreas_Tail","Pancreas_Body",
  "Pancreas_Head","Whole_Blood", "Spleen", "Muscle", "Skin"
)
df_plot$Var1<-factor(df_plot$Var1,levels = order )
