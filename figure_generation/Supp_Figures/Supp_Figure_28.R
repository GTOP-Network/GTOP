...
plotdf <- fread("./input/Figure S28a.left.txt")
plotdf$QTL<-factor(plotdf$QTL,levels = c("tuQTL novel","tuQTL known"))
p1 <- plotdf %>%
...
# Supp.Fig.28a right known or novel tuVariants proportion -----------------


plotdf <- fread("./input/Figure S28a.right.txt")
...
# Supp.Fig.28b missense sQTL enrichment --------
plot <- fread("./input/Figure S28b.txt")
plot$ratio<-plot$ratio*100
plot$group<-factor(plot$group,levels = c("sQTL missense","Background missense"))
ggplot(plot |> dplyr::filter(inss10orSF), 
aes(x = group, y = ratio, fill = group)) +
  geom_col(width = 0.65) +
  theme_pubr() +
  labs(x = "",y = "Proportion within splice regulation region"
  ) +
  scale_fill_manual(values=c("Background missense" = "grey80", "sQTL missense" = "#6E4B6A"))+
  theme(
    legend.position = "none"
  )+coord_flip()

# Supp.Fig.20c distribution of credible set variants  --------

density_df <- fread("./input/Figure S28c.txt")

density_df %>% 
  ggplot(., aes(x = pos_index, color=qtltype)) +
  geom_density( alpha = 0.5, adjust = 0.5) +
  labs(x = "Position", y = "Density of sQTL") +
  scale_color_manual(values = c("SV" ="#477980" , "TR" = "#9d3928", "SNV" = "#7d8bad"))+
