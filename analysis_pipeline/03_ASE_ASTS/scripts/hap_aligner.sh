...
# Align and process pacbio reads with minimap2
...
# echo "Aligning to haplotype 1..."
(set -x; minimap2 -t ${THREADS} -ax splice:hq -uf ${REFERENCE}_hap1.fa ${FASTQ} ${REVERSE} | \
     samtools view -q ${MIN_MAPQ} -F 2304 -Sb | \
     samtools sort -@ ${THREADS} - -o ${NAME}_reads_aln_sorted.hap1.bam)
# echo "Aligning to haplotype 2..."
(set -x; minimap2 -t ${THREADS} -ax splice:hq -uf ${REFERENCE}_hap2.fa ${FASTQ} ${REVERSE} | \
     samtools view -q ${MIN_MAPQ} -F 2304 -Sb | \
