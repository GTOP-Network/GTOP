...
genotype = paste0(dir,'/eVar_GT.vcf')
phenotype = paste0(dir,'/expr.phen')

# output file prefix with path info
prefix = tools::file_path_sans_ext(genotype)

X = t(read.table(genotype, header=T, row.names=1, quote="'", check.names=FALSE,sep="\t"))
class(X) <- "numeric"
...
y = read.table(phenotype, head=F,sep="\t")[,-1]
