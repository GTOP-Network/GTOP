...
for(idx in 1:length(gene_list)){
        file_name <- paste0("./output/TR/",opt$tissue,"/",gene_list[idx],"/eTR_GT.SuSiE.txt")
        cat(file_name,"\n")
...
write.table(susie_df,file=paste0("./SuSiE_Summary/susieR_res.tr_ju.",opt$tissue,".txt"),quote=F,row.names=F,sep="\t")
