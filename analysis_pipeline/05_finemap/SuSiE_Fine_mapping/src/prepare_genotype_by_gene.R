...
genotype_bed <- "./input/genotype_dosage/GTOP_SNV_INDEL_LRS.genotype_dosage.RDS"
genelist <- paste0("./input/tissue_gene_snv/",opt$tissue,"_gene_list.txt")
...
for(i in 1:dim(df_genes)[1]){
	...
	snp_file <- paste0("./output/Phase_I/Finemapping_eQTL_snv/",opt$tissue,"/",gene,"/snp_list.txt")
	df_snp <- fread(snp_file,header=F,sep="\t")
	snp_list <- df_snp$V1
	rm(df_snp)
	df_gt <- df_geno %>% filter(variant_id %in% snp_list)
	outdir <- paste0("./output/Phase_I/Finemapping_eQTL_snv/",opt$tissue,"/",gene)
