...
ref_pairs_df = pd.read_csv(args.extract_pairs, sep='\t', usecols=['variant_id', 'phenotype_id', 'chr'], dtype={'variant_id':str, 'phenotype_id':str, 'chr':str}, memory_map=True)
...
for f in file_paths:
    print(f)
    for i,chunk in enumerate(pd.read_csv(f, sep='\t', iterator=True, chunksize=1000000, index_col=0,
        dtype={'phenotype_id':str, 'rsid':str, 'variant_id':str, 'tss_distance':np.int32, 'start_distance': str,
            'ma_samples':np.int32, 'ma_count':np.int32, 'af':str,
