...
def get_introns(intron_counts_df):
    intron_df = pd.DataFrame([i.split(':') for i in intron_counts_df.index],
                             columns=['chr', 'start', 'end', 'clu', 'class'])
...
def map_cluster_to_genes(intron_df, exon_df, strict=True, verbose=False):
    """
    Modified version:
    Establish mapping relationships based only on basic cluster ID, excluding class information
    Add strand consistency check to ensure cluster strand matches gene strand
    """
    matches_df = []
    num_unmatched = 0
    num_unpaired = 0
    num_strand_mismatch = 0
    
    # pick strand info from cluster ID
    intron_df['cluster_strand'] = intron_df['clu'].str.extract(r'.*_([+-])$')[0]
    ...
    for c in sorted(intron_df['chr'].unique()):
        ...
        # add checking strand
        if 'strand' in exons_chr.columns:
            # sum cluster with not match strand before filtering
            all_matches_before_strand = pd.concat([five_prime_matches, three_prime_matches])
            if len(all_matches_before_strand) > 0:
                strand_mismatched = all_matches_before_strand[
                    all_matches_before_strand['cluster_strand'] != all_matches_before_strand['strand']
                ]
                num_strand_mismatch += strand_mismatched['clu'].nunique()
            
            # filter strand not match
            five_prime_matches = five_prime_matches[
                five_prime_matches['cluster_strand'] == five_prime_matches['strand']
            ]
            three_prime_matches = three_prime_matches[
                three_prime_matches['cluster_strand'] == three_prime_matches['strand']
            ]
        ...
        if strict:
            # only assign clusters that have both 5' and 3' matches to the same gene AND same strand
    ...
    if len(matches_df) > 0:
        ...
        clu_s = pd.Series(dtype=str, name='genes')
    
    if verbose:
        total_clusters = intron_df['clu'].nunique()
        matched_clusters = len(clu_s) if len(clu_s) > 0 else 0
        print(f"Total clusters: {total_clusters}")
        print(f"  * with ≥1 match: {matched_clusters} ({matched_clusters/total_clusters*100:.2f}%)")
        ...
        if 'strand' in exon_df.columns:
            print(f"  * strand mismatch: {num_strand_mismatch} ({num_strand_mismatch/total_clusters*100:.2f}%)")
...
def filter_counts(counts_df):
    ...
    cluster_ids = np.unique(counts_df.index.map(lambda x: x.split(':')[-2]))
    filtered_cluster_ids = np.unique(filtered_counts_df.index.map(lambda x: x.split(':')[-2]))
...
if __name__=='__main__':
    ...
    parser.add_argument('--geneinfo', help='gene definitions file, with columns: DDX11L16        lncRNA  ENSG00000290825.2       chr1    11121   24894   +')
    ...
    gene_bed_df = pd.read_csv(args.geneinfo, sep='\t', header=None)
    gene_bed_df.columns = ['gene_name', 'gene_type', 'gene_id', 'chr', 'start', 'end', 'strand'] + [f'extra_col_{i}' for i in range(7, gene_bed_df.shape[1])]
    ...
    counts_df = pd.read_csv(os.path.join(args.output_dir, f'{args.prefix}.cluster_ratios.gz'), sep='\s+', index_col=0) 
    pattern = r'^chr([1-9]|1\d|2[0-2]|X)[:\-]'
    counts_df = counts_df[counts_df.index.str.contains(pattern, regex=True)]
    # change columns to sample IDs
    # col_dict = {i:i.split('.')[0] for i in counts_df.columns}
    col_dict = {i: '-'.join(i.split('-')[:2]) for i in counts_df.columns}
    ...
    clu_s = map_cluster_to_genes(intron_df, exon_df, strict=False, verbose=True)
    ...
    group_s = {}
    no_gene_mapping_df = []
    no_gene_mapping_file = os.path.join(args.output_dir, f'{args.prefix}.no_gene_mapping.intron.txt')

    #gene mapping
    for _, r in bed_df.iterrows():
        s = r['ID'].split(':')

        base_cluster_id = s[0] + ':' + s[-2]
        
        if base_cluster_id in cluster2gene_dict:
            gene_ids = cluster2gene_dict[base_cluster_id].split(',')
            for g in gene_ids:
                gi = r['ID'] + ':' + g
            ...
            n += 1
            no_gene_mapping_df.append(r.tolist())
            gene_bed_df.append(r.tolist())
